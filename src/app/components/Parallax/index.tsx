...
export default function Parallax(props: PropsParallax) {
  ...
  const handleScroll = () => {
    ...
    if (position > 200) setScrollPosition(true);

    if (position > 300) setScrollPosition1(true);

    if (position > 380) setScrollPosition2(true);

    if (position > 420) setScrollPosition3(true);
  };
    ...
    <div className={`${styles.main}`}>
      <section className={`${styles.banner}`}>
        <div className={`${styles.divButtons} ${font.className}`}>
          <button onClick={scrollToIntroduction}>
            {props.selectedLanguage == "Pt-BR" ? "Introdução" : "Introduction"}
          </button>
          <Link href="/docs" legacyBehavior>
            <button>
              {props.selectedLanguage == "Pt-BR" ? "Iniciar" : "Get Started"}
            </button>
          </Link>
        </div>
      </section>
      <section
        className={`${styles.container} ${fontOxygen.className}`}
        id="introduction-section"
      >
        <div>
          <h2
            style={{
              opacity: scrollAnimation ? 1 : 0,
